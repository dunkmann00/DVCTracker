...
def send_email(subject, email_message, email_addresses, html_message=True):
    ...
    response = requests.post(
        ...
        data={
            "from": f"DVC Tracker <mailgun@{current_app.config['MAILGUN_DOMAIN_NAME']}>",
            "to": email_addresses,
...
def send_update_email(email_message, user):
    ...
    if len(email_addresses) == 0:
        print(
            f"No email addresses associated with {user}. Not sending update email."
        )
...
def send_error_email(email_message, html_message=True):
    ...
    if len(email_addresses) == 0:
        ...
        return NotificationResponse.Success
    return send_email(
        "DVC Tracker Error", email_message, email_addresses, html_message
    )
...
def send_error_report_email(email_message, html_message=True):
    ...
    if len(email_addresses) == 0:
        ...
        return NotificationResponse.Success
    return send_email(
        "DVC Tracker Error Report", email_message, email_addresses, html_message
    )
