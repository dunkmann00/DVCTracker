...
from ...util import ContactTypes, SpecialTypes
from .fields import EmailField, HiddenIntegerField, MultiCheckboxField, TelField
from .validators import RequiredIf, RequiredWhen, Tel

FieldTuple = namedtuple("FieldTuple", ["data"])


class ImportantCriteriaForm(Form):
    special_type = SelectField(
        "Special Type",
        [Optional()],
        choices=[
            (SpecialTypes.PRECONFIRM, "Preconfirmed Reservation"),
            (SpecialTypes.DISC_POINTS, "Discounted Points"),
        ],
        coerce=SpecialTypes,
        default=SpecialTypes.PRECONFIRM,
    )
    ...
    check_out_date = DateField(
        ...
        [
            RequiredIf(
                "check_in_date",
                message="This field is required when setting a Check In date.",
            )
        ],
    )
    length_of_stay = IntegerField(
        "Length of Stay (Nights - Minimum)",
        [
            Optional(),
            NumberRange(min=1, max=30, message="Must be between 1 and 30."),
        ],
    )
    points = IntegerField(
        "Points (Minimum)",
        [Optional(), NumberRange(min=1, message="Must be greater than 0.")],
    )
    price = IntegerField(
        "Price (Maximum)",
        [Optional(), NumberRange(min=1, message="Must be greater than 0.")],
...
class ImportantCriteriaListForm(FlaskForm):
    important_only = RadioField(
        "Send Important Updates Only",
        [InputRequired()],
        choices=[(False, "No"), (True, "Yes")],
        coerce=coerce_bool,
    )
    important_criteria = FieldList(
        FormField(ImportantCriteriaForm), min_entries=1
    )
    ...
    def from_json(cls, json):
        ...
        criteria_list_form = cls(
            data={
                "important_criteria": criteria_list,
        ...
        if not self.new_contact:
            ...
            if not user.is_valid_contact_id(field.data, self.contact_type):
                raise ValidationError(
                    "There was a problem removing this contact. Please try again later."
                )
    ...
    def validate_email_address(self, field):
        if self.new_contact:
            ...
            if not user.is_new_contact(
                field.normalized_data, self.contact_type
            ):
...
class PhoneForm(ContactForm):
    ...
    def to_json(self):
        ...
        json["phone_number"] = (
            ...
            if not self.errors
            else self.phone_number.raw_phone_data
        )
        return json
...
class ContactListForm(Form):
    ...
    def get_first_form(self, contact_type):
        ...
        contact_forms = (
            self.email_forms
            if contact_type is ContactTypes.EMAIL
            else self.phone_forms
        )
        if len(contact_forms) > 0:
            return contact_forms[0].form
        return (
            EmailForm() if contact_type is ContactTypes.EMAIL else PhoneForm()
        )
