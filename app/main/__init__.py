...
def static_url(filename, _anchor=None, **kwargs):
    ...
    if use_static_server and not current_app.config["STATIC_SERVER_NAME"]:
        raise RuntimeError(
            "Building static url requires 'STATIC_SERVER_NAME' not be none."
        )
    server_name = (
        current_app.config["STATIC_SERVER_NAME"]
        if use_static_server
        else request.host
    )
    script_name = (
        current_app.config["STATIC_DIRECTORY"] if use_static_server else "/"
    )
    url_scheme = (
    ...
    urls = current_app.url_map.bind(
        server_name, script_name=script_name, url_scheme=url_scheme
    )
